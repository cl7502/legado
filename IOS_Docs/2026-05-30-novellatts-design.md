...
```
模式1: 「name + 说/道/问/答/喊/叫/笑/哭/骂/低声/大声/沉声道」+ 引号
模式2: 引号内容 + 「name + 说/道」
模式3: 「name：「内容」」（书名号格式）
```

说话动词词典可配置扩展。未命中 → 返回 nil → 交给 LLM。
...
| ID | 名称 | Kokoro SpeakerID | 适用场景 |
|----|------|-----------------|---------|
| `narrator` | 旁白 | `zf_001` | 中性女声旁白 |
| `protagonist` | 男主 | `zm_001` | 青年男声 |
| `heroine` | 女主 | `zf_002` | 青年女声 |
| `elder_male` | 老者 | `zm_003` | 成熟男声 |
| `young_female` | 少女 | `zf_003` | 清甜女声 |
| `villain` | 反派 | `zm_002` | 低沉男声 |
| `child` | 孩童 | `zf_004` | 童声 |
| `neutral` | 通用 | `zm_004` | 中性备用 |
